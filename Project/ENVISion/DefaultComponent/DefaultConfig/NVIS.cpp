...
//## class NVIS
//#[ ignore
NVIS::port_NVIS_C::port_NVIS_C(void) : IB_NVIS(), _p_(0), itsIB_NVIS(NULL) {
}

NVIS::port_NVIS_C::~port_NVIS_C(void) {
    cleanUpRelations();
}

IB_NVIS* NVIS::port_NVIS_C::getItsIB_NVIS(void) {
    return this;
}

IB_NVIS* NVIS::port_NVIS_C::getOutBound(void) {
    return this;
}

double NVIS::port_NVIS_C::runCar(void) {
    double res = 0.0;
    if (itsIB_NVIS != NULL) {
...
void NVIS::port_NVIS_C::setItsIB_NVIS(IB_NVIS* const p_IB_NVIS) {
    itsIB_NVIS = p_IB_NVIS;
}

void NVIS::port_NVIS_C::cleanUpRelations(void) {
    if(itsIB_NVIS != NULL)
        {
            itsIB_NVIS = NULL;
        }
...
NVIS::NVIS(IOxfActive* const theActiveContext) : OMReactive(), activeMode(4), fogLamp(true), fogMode(true), headLights(false), normalMode(false), speed(80), systemOk(false), track1Mode(false), track2Mode(false), winding(false), windingMode(false), itsCar(NULL), itsCommunicationSystem(NULL), itsConstraints(NULL), itsHeadlights(NULL), itsHighResVideoCam(NULL), itsInfraredFilter(NULL), itsMonitoringScreen(NULL), itsNaturalEnvironment(NULL), itsStakeholder(NULL), itsStandards(NULL) {
...
NVIS::port_NVIS_C* NVIS::getPort_NVIS(void) const {
    return (NVIS::port_NVIS_C*) &port_NVIS;
}

NVIS::port_NVIS_C* NVIS::get_port_NVIS(void) const {
    return (NVIS::port_NVIS_C*) &port_NVIS;
}
...
const double NVIS::getSpeed(void) const {
    return speed;
}

void NVIS::setSpeed(const double p_speed) {
    speed = p_speed;
}
